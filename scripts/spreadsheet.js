...
evaluateGroupSum = (item) => {
  ...
  let valueArr = [];
  let cellExpression = item.innerHTML;
  let c = cellExpression.replace("=SUM","").replace("(", "").replace(")", "").split(":");
  ...
  if(!(colHeaderArr.includes(startCol) &&  colHeaderArr.includes(endCol) && startRow>=1 && endRow <= spreadSheetData.length)){
  ...
  if(startCol == endCol && startRow != endRow){
    ...
    for(let i = startRow; i<=endRow; i++){
      valueArr.push(checkCellValueFormat(String(document.getElementById(`${startCol}${i}`).innerHTML)));
      cell.observables.push(this.getCellObs(String(startCol+i)));
      }
    }
  else if(startCol != endCol && startRow == endRow){
    console.log("row group sum");
    for(let j = colHeaderArr.indexOf(startCol)+1; j<=colHeaderArr.indexOf(endCol)+1; j++){
      valueArr.push(checkCellValueFormat(String(document.getElementById(`${colHeaderArr[j-1]}${startRow}`).innerHTML)));
      cell.observables.push(this.getCellObs(String(colHeaderArr[j-1]+startRow)));
      }
    }
  else{
...
console.log("evaluateMathEqn")
  ...
  for(let i = 0; i<cellIndices.length;i++){
      ...
      //check boundary condition and if headers are excluded
      if(!(colHeaderArr.includes(colIndex[0]) && rowIndex[0]>=1 && rowIndex[0] <= spreadSheetData.length)){
        console.log("Boundary condition error")
        return error;
  }
}
...
removeObs(cell);
  for(let i = 0; i< eqn.length;i++){
    if(cellIndices.includes(eqn[i])){
      ...
      console.log(rowIndex[0]+" "+colIndex[0]);
      valueArr.push(checkCellValueFormat(String(document.getElementById(`${colIndex}${rowIndex}`).innerHTML)));
...
//check self reference func
  ...
  checkCellValueFormat = (value) => {
    if(value == "!ERR"){
      return "!ERR"
    }else if(isNaN(value)|| value == "" || value.match(/[\s{?}]/gi)|| value.match(/[a-zA-Z]/gi)){
      return 0;
    }else{
      return value;
    }
...
function updateMessages(formula, displayValue) {
...
function removeObs(cell){
  if (cell.observables.length>0) {
    ...
    console.log("Observables exist but formula is removed, unsubscribe and delete")
...
getspreadSheetData = () => {
    if (spreadSheetData === undefined || spreadSheetData === null || spreadSheetData.length === 0) {
...
displaySpreadSheet = (op) => {
    ...
    let table="";
    if(op === "update"){
      const spreadSheetData = this.getspreadSheetData();
    ...
    if (e.target && e.target.nodeName === "TD") {
      ...
      let item = e.target;
      let indices = getIndices(item);
      updateMessages(spreadSheetData [indices[0]][colHeaderArr.indexOf(indices[1]) + 1].formula, spreadSheetData [indices[0]][colHeaderArr.indexOf(indices[1]) + 1].displayValue);
      if(spreadSheetData [indices[0]][colHeaderArr.indexOf(indices[1]) + 1].formula!=""){
        item.innerHTML = spreadSheetData [indices[0]][colHeaderArr.indexOf(indices[1]) + 1].formula;
      }
      else{
        item.innerHTML = spreadSheetData [indices[0]][colHeaderArr.indexOf(indices[1]) + 1].displayValue;
      }   
      }
  });
...
getTableData = (item) => {
    ...
    for (let i = 1; i <=spreadSheetData.length-1; i++) {
      for (let j = 1; j <=spreadSheetData[i].length-1; j++) {
        ...
        if (spreadSheetData[i][j].formula && spreadSheetData[i][j].formula.startsWith("=")) {
            console.log("check if formula exists")
          if (spreadSheetData[i][j].observables.length>0) {
            console.log("check if observables exists ")
            spreadSheetData[i][j].observables.forEach(observable => { observable
              .subscribe(() => {
...
selectCol = colId => {
  let spreadSheetData = this.getspreadSheetData();
...
addColumnBtn.addEventListener("click", function () { addCol((this)) });
addRowBtn.addEventListener("click", function () { addRow((this)) });
delColumnBtn.addEventListener("click", function () { delCol((this)) });
delRowBtn.addEventListener("click", function () { delRow((this)) });
exportBtn.addEventListener("click", function () { exportCSV((this)) });
importBtn.addEventListener("click", function () { importCSV((this)) });

disableButtons = (value) => {
  // addColumnBtn.disabled = value;
  // addRowBtn.disabled = value;
  // delColumnBtn.disabled = value;
  // delRowBtn.disabled = value;
    ...
    if ((cell.formula || "").startsWith("=")) {
      ...
      if(operation == "addCol"){
        splittedAcutalValue.forEach(function(item, index) {
          if (selectedRowOrColumnIndex <= colHeaderArr.indexOf(item) && colHeaderArr.indexOf(item) >= 0 &&colHeaderArr.indexOf(item) < spreadSheetData[0].length - 1) {
      ...
      if(operation == "addRow"){
        splittedAcutalValue.forEach(function(item, index) {
          if (parseInt(item) > selectedRowOrColumnIndex && parseInt(item) > 0) {
      ...
      if(operation == "delCol"){
        splittedAcutalValue.forEach(function(item, index) {
          if (
            ...
            let desiredIndex = colHeaderArr.indexOf(item) - 1;
            this[index] = colHeaderArr[desiredIndex] || "";
          } else if (colHeaderArr.indexOf(item) === selectedRowOrColumnIndex - 1) {
...
const importCSV = () => {
 console.log("importCSV Function");
